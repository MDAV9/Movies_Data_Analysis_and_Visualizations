...
movies_omdb = pd.read_csv("Data/omdb_movies_data.csv", low_memory=False)
...
st.markdown("""In this App we spectacle **Movies** data for **Analysis** and **Visualization**
* **Python libraries:**streamlit, pandas, numpy, matplotlib, wordcloud, requests, plotly, Image
* **Data source:** [IMDB](https://datasets.imdbws.com/), [OMDB](https://www.omdbapi.com/), [TMDB](https://developers.themoviedb.org/3/movies/get-movie-details), [BoxOffice Mojo](https://www.boxofficemojo.com/year/world/)""")
...
def MDAV_options():
    ...
    def Top_10_geners():
        try:
            ...
            **Drama** is more and for **USA** movies the **Documentary** and **Drama** are equally overlaped at each 
            ...
            df_top_10 = df_series.loc[:, ['Drama', 'Comedy', 'Thriller', 'Romance', 'Action', 'Horror', 'Documentary']]
            df_top = df_top_10.reset_index()
            df_top.head()
            fig1 = px.line(df_top, 
                x = 'Year', 
                y = ['Drama', 'Comedy', 'Thriller', 'Romance', 'Action', 'Horror', 'Documentary'], 
    ...
    def Maximum_Rated_Movies():
        try:
            ...
            slider_range = st.slider("Select the range to visualize", 1900, 2020 , (1900, 2020))
            smalldf = df[df['Year'] > slider_range[0]]
            small_df = df[df['Year'] < slider_range[1]]
            small_df = smalldf[['Title', 'Year', 'Rating']]
    ...
    def Crew_movies_count():
        try:
            st.info("""We have visualized The 20 known **Directors**, **Production Houses**, **Actors** and **Actress**, movies count 
            ...
            **Mickey Rooney** and actress is **Katharine Hepburn**""")
            story_select = st.selectbox(
            label = "Try all options",
            options = ['Select', 'Actors', 'Director', 'Production'])
            if story_select == 'Actors':
                ...
                fig2 = px.line(df2, x="Actor", y="Number_of_Movies", title='Actress and their Movies Count')
                st.plotly_chart(fig)
                st.plotly_chart(fig2)
    ...
    def StripPlots_based_on_Year():
        try:
            ...
            and, Popularity**. For these attributes we have fixed some limits which are **Ratings > 5**, **Votes > 200**, **Budget, Collections, Profit > 10000**
            ...
            story_select = st.selectbox(
            label = "Try all options",
            options = ['Select','Ratings', 'Votes','Budget', 'Collections', 'Profit', 'Popularity'])
            if story_select == 'Ratings':
                df25 = df[df['Year'] > 1999]
                df25 = df25[df25['Year'] < 2022]
                df25 = df25.loc[df25["Rating"] > 5]
            ...
            if story_select == 'Votes':
                df25 = df[df['Year'] > 1999]
                df25 = df25[df25['Year'] < 2022]
                df25 = df25.loc[df25["Votes"] > 200.0]
...
(https://share.streamlit.io/madhuri97/streamlit_app_105/main/data_app_final.py).</small>''', unsafe_allow_html=True)